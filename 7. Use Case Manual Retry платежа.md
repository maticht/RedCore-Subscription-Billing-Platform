...
## Supporting actors
Retry Service, Payment Service, Provider Adapter, Kafka, UI (Admin Panel), Audit Log, Notification Service
...
## Основной (счастливый) сценарий — Main Success Scenario
...
10. При успешном ответе от провайдера:
    - Payment Service обновляет запись payment → status = `succeeded`, attempts_count++  
    - Публикуется событие `payments.succeeded`  
    - UI получает обновление (websocket/poll) и показывает уведомление "Платёж успешно проведён"  
    - Audit Log записывает: admin_id, payment_id, task_id, attempt_number, result = success, timestamp  
11. При неуспешном ответе провайдера (временная ошибка):
    - Payment Service увеличивает attempts_count, возможно устанавливает next_attempt_time согласно retry policy  
    - Если attempts_count < max_retries публикуется `payments.retry.requested` и статус остаётся `failed` (или `retrying`)  
    - Если attempts_count reached max_retries статус = `failed_permanent` и публикуется `payments.failed_permanent`  
    - UI обновляет карточку и показывает текущее состояние  
    - Audit Log фиксирует результат и сообщение провайдера

## Альтернативные сценарии / Extensions
- **A1: Недостаточно прав**
  - При шаге 6 API возвращает 403 Forbidden и не публикует событие
  - UI показывает ошибку доступа

- **A2: Платёж уже в succeeded или failed_permanent**
  - При шаге 6 API возвращает 409 Conflict с пояснением и не создаёт задачу
  - UI показывает сообщение "Retry невозможен для текущего статуса"

- **A3: Idempotency / повторный клик**
  - Если админ случайно нажал retry несколько раз, API проверяет idempotency (Idempotency-Key) или task_id и возвращает существующий task_id без создания новой задачи
  - Audit Log фиксирует повторную попытку запуска от того же admin_id

- **A4: Сбой коммуникации с Retry Service / Kafka**
  - При проблемах публикации API возвращает 502/503 и сохраняет временную заявку в локальном queue для повторной отправки
  - UI показывает сообщение "Ошибка, retry отложен"

- **A5: Провайдер ответил permanent failure**
  - Payment Service помечает payment = `failed_permanent`, публикует `payments.failed_permanent`, UI уведомляет админа
